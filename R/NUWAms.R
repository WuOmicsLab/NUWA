#' Infer proteome expression abundance for missing immune markers.
#'
...
#'
#' @param expr a numeric matrix or data frame of proteome expression profiles for
#' bulk samples, with HUGO gene symbols as rownames and sample identifiers as colnames.
#' @param network a list, consisting of the co-expression networks built by function
...
#' be used followingly.
#' @param direction a character, indicating the mode used for feature searching in
#' stepwise regression analysis, one of "both", "backward" or "forward". Default is "both".
#' @param lasso_step_cutoff a positive integer, specifying the minimal number of variables
...
#' @param preprocess logical. If TURE, expression data is preprocessed before markers inferring.
#' Default is TRUE. See the Methods section of the NUWA manuscript for more details.
#' @param ncores a positive integer, indicating the number of cores used by this function.
#' If the operating system is windows, then only one core will be used.
#' @param lambda a character, indicating which value of lambda will be used in the LASSO
...
NUWAms <- function(expr, network = NULL, direction=c("both", "backward", "forward")[1],
    ...
    finalRes <- list(
        ...
        inferrenceMat = reverse_fun(expr.fill),
