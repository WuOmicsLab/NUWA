...
#' @param thre A threshold ranging from 0 to 100. when use.sams is False, genes with a non-NA value proportion below thre% will be filtered out. Default 50.
...
#' res <- preprocess(expr)
preprocess <- function(expr, idcorr = F, quantile_normalization = T, batchInfo = NULL, 
                       use.sams = F, thre = 50, nsams = 10, usecap = T
                       ) {
    ...
    quantile_by_batch <- quantile_normalization
